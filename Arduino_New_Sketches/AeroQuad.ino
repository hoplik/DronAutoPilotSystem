...
#endif
#include "OnBoardPilotSystem.h"	// Автопилот
...
#else
#define SERIAL_PORT Serial
//#define SERIAL1_PORT Serial3	// Порт связи с нано (temporary disabled)
#endif
#endif  
...
void setup() {
	SERIAL_BEGIN(BAUD);
	//SERIAL1_BEGIN(BAUD); Пока закомментируем, т.к. изменён способ подключения наны
	pinMode(LED_Green, OUTPUT);
...
void process10HzTask2() {
	...
	readSerialCommand();
	//serialEvent(); Пока закомментируем, т.к. изменили подключение наны к шилду
	sendSerialTelemetry();
}
