...
/***********************************************
*	Modified 21.11.2015 by HOPLIK SYSTEMS  *
...
void readSerialCommand() {
	...
	if (SERIAL_AVAILABLE()) {
		...
		case 'T': // Send individual transmitter values (axes, value). For OnBoardPilotSystem ONLY!!!
			if (OnBoardPilotSystem == ON) {
				for (byte axes = 0; axes < LASTCHANNEL; axes++) {
					PilotSystemCommand[axes] = (int)readFloatSerial();
				}
			}
			break;
...
/* Пока закомментируем, изменили подключение наны
...
void serialEvent()	// Читаем данные наны и присваиваем массиву дальномера
{
	if (SERIAL1_AVAILABLE()){
		rangeFinderRange[0] = readFloatSerial1() / 100;
		//rangeFinderRange[1] = readFloatSerial1() / 100;
		//rangeFinderRange[2] = readFloatSerial1() / 100;
		//rangeFinderRange[3] = readFloatSerial1() / 100;
		//rangeFinderRange[4] = readFloatSerial1() / 100;
	}
}
*/
//***************************************************************************************************
...
void sendSerialTelemetry() {
	...
	case 'S': //Отдаём только то, что нужно для навигации
		PrintValueComma(kinematicsAngle[XAXIS]);
		PrintValueComma(kinematicsAngle[YAXIS]);
	...
	case 'z': // Send all Altitude data 
		...
		for (int i = 0; i < RANGER_COUNT; i++){
			PrintValueComma(rangeFinderRange[i]);
}
#else
		PrintValueComma(0);
#endif 	
