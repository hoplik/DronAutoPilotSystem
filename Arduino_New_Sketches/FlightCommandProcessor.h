...
/***********************************************
*	Modified 22.02.2015 by HOPLIK SYSTEMS  *
...
void processZeroThrottleFunctionFromReceiverCommand() {
	...
	// Switch from transmitter commands to OnBoardPilotSystem
	// ГАЗ - ВНИЗ и ВПРАВО; ROLL - вниз и вправо - включение автопилота
	if ((receiverCommand[ZAXIS] > MAXCHECK) && (receiverCommand[XAXIS] > MAXCHECK) && (receiverCommand[YAXIS] < MINCHECK))
	{
		OnBoardPilotSystem = ON;	// Включили автопилот
		digitalWrite(LED_Red, HIGH);	// Зажгли красный диод
		SERIAL_PRINTLN("SOBPS_ON");	// Отправили в порт подтверждение включения автопилота
	}
	// Switch from OnBoardPilotSystem to fransmitter commands
	// Газ - вниз и вправо; ROLL - вниз и влево - выключение автопилота
	if ((receiverCommand[ZAXIS] > MAXCHECK) && (receiverCommand[XAXIS] < MINCHECK) && (receiverCommand[YAXIS] < MINCHECK))
	{
		OnBoardPilotSystem = OFF;	// Выключили автопилот
		digitalWrite(LED_Red, LOW);	// Погасили красный диод
		SERIAL_PRINTLN("SOBPS_OFF");	// Отправили в порт подтверждение отключения автопилота
	}
}
...
void readPilotCommands() {
	...
	/*
	  Подменяем данные приёмника на данные из порта, если включён автопилот
	  */
	if (OnBoardPilotSystem == ON)
	{
		for (int i = 0; i < LASTCHANNEL; i++)
		{
			receiverCommand[i] = PilotSystemCommand[i];
		}
	}
